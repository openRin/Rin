...
export function Settings() {
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((err: any) => {
        showAlert(
          t("settings.get_config_failed$message", { message: err.message })
        );
      })
      .finally(() => {
  ...
  function onFileChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (file) {
      client.wp
        ...
        .catch((err) => {
          showAlert(
            t("settings.import_failed$message", { message: err.message })
          );
        });
    }
  }
  ...
  return (
    <div className="flex flex-col justify-center items-center test">
            ...
            <ul className="flex flex-col items-start max-h-64 overflow-auto w-full">
              {msgList.map((msg, idx) => (
                <p key={idx} className="text-sm dark:text-white">
                  {t("settings.import_skipped_item$title$reason", {
                    title: msg.title,
                    reason: msg.reason,
                  })}
                </p>
...
function ItemSwitch({
  ...
  function updateConfig(type: "client" | "server", key: string, value: any) {
    ...
    client
        ...
        if (type === "client") {
          ...
          if (config) {
            sessionStorage.setItem(
              "config",
              JSON.stringify({ ...JSON.parse(config), [key]: value })
            );
          } else {
      ...
      .catch((err) => {
        showAlert(
          t("settings.update_failed$message", { message: err.message })
        );
        setChecked(checkedValue);
        setLoading(false);
      });
